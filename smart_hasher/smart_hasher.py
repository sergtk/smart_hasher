...
def get_hash_file_name_postfix():
    ...
    if cmd_line_args.add_output_file_name_timestamp:
        postfix += "." + start_time_dict["file_postfix"]
...
def handle_input_files(hash_storage: hash_storages.HashStorageAbstract):
    ...
    header_comments = [
        ...
        f"Timestamp of hash calculation: {start_time_dict['str']}",
        f"Hash algorithm: {cmd_line_args.hash_algo}"]
    if cmd_line_args.user_comment:
        header_comments = header_comments + [f"User comment: {cmt}" for cmt in cmd_line_args.user_comment]
...
def fill_start_time_dict():
    ...
    start_time_dict["str"] = start_time_dict["datetime"].strftime("%Y-%m-%d %H:%M:%S") + f" {tz_str}";
